...
scripts_dir = '/raid1/scratch/kelvinw/auto_selfcal'
print(f"You have appended the script directory: {scripts_dir} to path")
if scripts_dir not in sys.path:
    sys.path.append(scripts_dir)

import argparse

parser = argparse.ArgumentParser(description="Run self-calibration on MS files")
parser.add_argument('--ms-dir', type=str, required=True, help="Path to the directory containing the Measurement Sets (MS)")

args = parser.parse_args()
ms_directory = args.ms_dir
print(f"Changing working directory to: {ms_directory}")
os.chdir(ms_directory)
